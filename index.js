...
const searchCity = async () => {
  ...
  try {
    ...
    tempResult.innerText = `🌡 ${Math.floor(
      parseInt(temprature - 273.15) * 1.8 + 32
    )}℉`;
    ...
    minT.innerText = `Low: ${Math.floor(
      parseInt(minTemp - 273.15) * 1.8 + 32
    )}℉`;
    ...
    minH.innerText = `High: ${Math.floor(
      parseInt(highTemp - 273.15) * 1.8 + 32
    )}℉`;
    ...
    feelsL.innerText = `Feels Like: ${Math.floor(
      parseInt(feelsLike - 273.15) * 1.8 + 32
    )}℉`;
